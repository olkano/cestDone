...
export function buildInitialWorkerInstructions(plan: Plan, phase: Phase, completedPhases: Phase[], env?: EnvironmentInfo): string {
  ...
  parts.push(
    ...
    '',
    'Read the phase spec above, explore the target repo, and execute.',
    'If the phase involves code changes, run tests at each step.',
    'Kill any servers or background processes when done.',
  )
...
export function buildReviewPrompt(phaseNumber: number, phaseName: string, phaseSpec: string, workerReport: string, runDir: string, completedSubPhases: string[] = [], autoCommit: boolean = true): string {
  ...
  parts.push(
    '',
    '## Task',
    'Review the Worker\'s work. You have a limited turn budget.',
    '- Read the diff or changed files ONCE. Do not re-read them.',
    '- The Worker already performed a compliance self-check. Trust its report for items it confirmed.',
    '  Only spot-check items the diff does not clearly confirm.',
    '- Do not read reference documents, style guides, or other files the Worker already used.',
    ...
    '- If the spec names a #### Reference Component, compare the deliverables against those patterns. Deviations are a `fix`.',
    '- Compare each stated requirement against what was delivered. Spec deviations are a `fix`, not informational — only the human can approve deviations.',
    ...
    '',
    '### 3. Outcome Verification (only when needed)',
    'Only do this when the phase involves external operations or user-visible behavior',
    'that the Worker\'s report does not fully confirm.',
    'If you determine verification is needed:',
    '- Start any required servers or processes',
    '- Verify the behavior or outcome works as specified',
    '- **IMPORTANT: Kill all servers and background processes when done**',
    '',
    'Skip this step entirely if the Worker\'s report already confirms the outcomes.',
    '',
    ...(autoCommit
      ? [
        '## Git Commits',
...
export function buildDirectorExecutionPrompt(plan: Plan, phase: Phase, completedPhases: Phase[], env?: EnvironmentInfo): string {
  ...
  parts.push(
    ...
    phase.spec,
    '',
    'Implement this phase directly. Use your full tools to read the repo, make changes, and verify.',
    'If the phase changes code, run tests. Kill any servers or background processes when done.',
    '',
    '### Response',
...
const PLAN_FORMAT_TEMPLATE = [
  ...
  '- [ ] <requirements from the spec that must be followed exactly>',
  '- [ ] <mandated tools, formats, or conventions>',
...
export function buildPlanningWorkerPrompt(spec: FreeFormSpec, env: EnvironmentInfo | undefined, planPath: string): string {
  ...
  parts.push(
    ...
    '',
    'Guidelines:',
    '- Each phase should be a discrete, verifiable deliverable',
    ...
    '- For each phase, include a #### Compliance Checklist inside ### Spec. Extract from the spec: requirements that must be followed exactly, mandated tools/formats/conventions, quality or compliance requirements, and documentation obligations. The checklist is verified by both the Worker and the Director review',
    ...
    '',
    'Do NOT ask questions. Do NOT execute the spec. Only explore the target repo and write the plan file.',
  )
...
export function buildPlanningSystemPrompt(spec: FreeFormSpec, env?: EnvironmentInfo): string {
  const parts: string[] = [
    ...
    '4. Review deliverables and verify outcomes',
...
export function buildExecutionSystemPrompt(plan: Plan, completedPhases: Phase[], env?: EnvironmentInfo): string {
  const parts: string[] = [
    'You are the Director of cestDone, an AI-orchestrated development system.',
    'Your role is to review deliverables, verify outcomes, and guide implementation.',
  ]
